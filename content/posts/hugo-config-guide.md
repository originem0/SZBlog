...
在 `custom.css` 的 `:root` 中覆盖 PaperMod 原生变量，可以全站生效：

```css
:root {
    --primary: #111;           /* 正文字色 */
    --secondary: #444;         /* 副文本、时间、摘要 */
    --theme: #f8faff;          /* 页面背景 */
    --entry: #ffffff;          /* 卡片背景 */
    --border: #e0e7f1;         /* 边框 */
    --code-bg: #eef2f9;        /* 行内代码背景 */
    --code-block-bg: #f5f7fb;  /* 代码块背景 */
}
```

暗色模式用 `[data-theme="dark"]` 选择器单独覆盖：
...
然后在 `custom.css` 中用 `.chroma` 相关类名控制语法颜色。可以用 Hugo 命令生成主题：
